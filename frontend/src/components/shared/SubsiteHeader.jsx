...
export default function SubsiteHeader({ navItems, brandPrefix, brandSuffix, contactHref, accentClass = 'text-gold' }) {
      ...
      <div className="h-full max-w-[1400px] mx-auto px-6 lg:px-10 flex items-center justify-between gap-6">
        ...
        <nav className="hidden xl:flex items-center gap-7">
          {navItems.map((item) => (
            ...
            >
              {item.href ? (
                <Link to={item.href} className={`text-sm font-medium transition-colors ${isActive(item.href) ? 'text-gold' : 'text-white/85 hover:text-gold'}`}>
                  {item.label}
                </Link>
              ) : (
                ...
                {item.children && openDropdown === item.label && (
                  ...
                  >
                    <div className="bg-dark/97 backdrop-blur-md border border-white/10 shadow-2xl min-w-[240px] py-2">
                      {item.children.map((c) => (
                        <Link key={c.href} to={c.href} className="block px-5 py-3 text-sm text-white/75 hover:text-gold hover:bg-white/5 transition-colors">
                          {c.label}
                        </Link>
                      ))}
                    </div>
        ...
        <div className="flex items-center gap-4">
          <Link to={contactHref} className="hidden md:inline-block border border-gold text-gold px-5 py-2 text-xs tracking-widest uppercase hover:bg-gold hover:text-black transition-all duration-300">
            Bize Ulaşın
          </Link>
          <button className="xl:hidden text-white/85" onClick={() => setMobileOpen(true)} aria-label="menu">
            <Menu size={24} />
          </button>
        </div>
      </div>
        ...
        {mobileOpen && (
          ...
          >
            <div className="flex justify-between items-center h-16 px-6 border-b border-white/10">
              ...
              <button onClick={() => setMobileOpen(false)} className="text-white/85"><X size={24} /></button>
            </div>
