...
export default function SubsiteFooter({ brandPrefix, brandSuffix, basePath = '/', accentClass = 'text-gold', description }) {
  return (
    <footer className="bg-dark border-t border-white/10 pt-16 pb-8 px-6 md:px-16">
      <div className="max-w-[1400px] mx-auto grid grid-cols-1 lg:grid-cols-4 gap-12">
        <div>
          ...
          <p className="text-white/50 text-sm mt-5 max-w-xs leading-relaxed">{description}</p>
        ...
        <div>
          ...
          <ul className="space-y-3">
            <li><Link to={basePath} className="text-white/60 text-sm hover:text-white hover:translate-x-1 transition-all inline-block">Anasayfa</Link></li>
            <li><Link to={`${basePath}/blog`} className="text-white/60 text-sm hover:text-white hover:translate-x-1 transition-all inline-block">Bizden Haberler</Link></li>
            <li><Link to={`${basePath}/iletisim`} className="text-white/60 text-sm hover:text-white hover:translate-x-1 transition-all inline-block">İletişim</Link></li>
          </ul>
        </div>
